...
cas %>%
  ...
  geom_pointrange(aes(y = Index, x = M, xmin = L, xmax = U), position = position_dodge2(1)) + 
  facet_grid(. ~ Country) +
...
cas %>%
  ...
  geom_pointrange(aes(x = Index, y = M, ymin = L, ymax = U), position = position_dodge2(1)) + 
  facet_grid(. ~ Country) +
