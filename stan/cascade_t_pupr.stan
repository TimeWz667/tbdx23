data {
  ...
  int<lower=0> Case_Pu[n_t]; // notification counts
  int<lower=0> Case_Pr[n_t];
...
transformed parameters {
  ...
  for (i in 1:n_t) {
    ...
    nr_pub[i] = tp[i] / ppv * p_pub;
    nr_pri[i] = tp[i] / ppv * (1 - p_pub) * (1 - p_under);
    nr[i] = nr_pub[i] + nr_pri[i];
  }
}
model {
  prv0 ~ uniform(0, 1);
